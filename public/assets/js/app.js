class App {
        ...
        elem.submit(function (e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    object.showFormErrors(elem, errors, 'name');
    ...
    table(elem, func = function() {}) {
        const object = this;
        var action = elem.attr("data-action");
        var method = "GET";
        var table_id = elem.attr("data-table-id");

        var page = table_id ? object.getCookie(`table_page_${table_id}`) : null;
        var order = table_id ? object.getCookie(`table_order_${table_id}`) : null;
        var order_type = table_id ? object.getCookie(`table_ordertype_${table_id}`) : null;
        ...
        elem.dataTable({
            ...
            data: {
                p: (page ? page : 1),
                ord: (order ? order : ""),
                ordtype: (order_type ? order_type : "ASC")
            },
            dataType: "json",
            beforeSend: function () {
                elem.toggleClass("opacity-2");
            },
            success: function (response) {
                if(response.message) {
                    object.showMessage(response.message.message, response.message.type);
                ...
                if(response.content) {
                    ...
                    const msg = $(`
                        <div>
                            <div class="alert alert-info">Não encontramos nada para ser carregado!</div>
                            <div class="d-flex justify-content-around">
                                <button type="button" class="btn btn-outline-info btn-lg" refresh>Recarregar</button>
                            </div>
                        </div>
                    `);

                    msg.find("button[refresh]").click(function () {
                        elem.loadTable();
                    });
    ...
    showFormErrors(form, errors = null, attr = '') {
